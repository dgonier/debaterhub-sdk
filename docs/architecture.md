...
The debaterhub SDK provides two integration modes (Token-Only / Server-Managed) and two debate modes (`ai_human` / `ai_ai`) for running IPDA debates over a LiveKit data channel.
...
The `DebateConfig.debate_mode` field controls who speaks:

- **`"ai_human"`** (default) — one side is the human, one is the AI agent. `human_side` determines which. The human submits speeches and CX responses via `ManagedDebateSession` action methods.
- **`"ai_ai"`** — both sides are generated by the AI agent. No human input is expected. `human_side` is ignored. Use this for autonomous debate runs, evaluation harnesses, or spectator UIs.

Both modes emit the same event stream (`debate_ready → turn_signal → speech_text → ... → judge_result`), so the same `DebateEventHandler` works for both. The only difference: in `ai_ai` mode, never call `submit_speech` / CX submission methods. See [modes.md](modes.md#ai-vs-ai-autonomous-debates) for a full example.
